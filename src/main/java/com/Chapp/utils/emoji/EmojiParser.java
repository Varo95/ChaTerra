...
public class EmojiParser {
    ...
    private EmojiParser() {
        URL url = App.class.getResource(EMOJIONE_JSON_FILE);
        assert url != null;
        ...
        UNICODE_PATTERN = Pattern.compile(unicodeToHex.keySet().stream().map(Pattern::quote).collect(Collectors.joining("|")));
    }
    ...
    public String unicodeToShortname(String str) {
        return replaceWithFunction(str, UNICODE_PATTERN, (unicode) -> {
    ...
    public String shortnameToAscii(String str) {
        return replaceWithFunction(str, SHORTNAME_PATTERN, (shortname) -> {
            if (shortname == null || shortname.isEmpty() || (!emojiList.containsKey(shortname))) {
                return shortname;
            }
    ...
    public String asciiToShortname(String str) {
        return replaceWithFunction(str, ASCII_PATTERN, (ascii) -> {
